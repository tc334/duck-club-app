...
class DbManager:
    ...
    def __init__(self, host, port):
        ...
        self.schema = database.tables.schema.schema
        self.tables = {}
        for key in self.schema:
            self.tables[key] = database.tables.table.Table(self.schema[key])
    ...
    def add_row(self, table_name, row_data_dict):
        ...
        mySql_insert_query = f"INSERT INTO {table_name} ({col_names}) VALUES ({value_types[:-2]}) "

        try:
            self.my_cursor.execute(mySql_insert_query, insert_tuple)
            self.db.commit()
    ...
    def update_row(self, table_name, update_dict):
        ...
        mySql_insert_query = f"UPDATE {table_name} SET {set_str} WHERE id=%s"

        try:
            self.my_cursor.execute(mySql_insert_query, tuple(data_list))
            self.db.commit()
        except mysql.connector.Error as error:
